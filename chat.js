...
const ChatManager = {
  ...
  async init(vehicleId = null) {
    console.log(
      '🗨️ ChatManager: Initializing',
      vehicleId ? `for vehicle ${vehicleId}` : 'general chat',
    );
      ...
      if (existingSessionId) {
        ...
        try {
          ...
          if (state && !state.error) {
            ...
            const greeting =
              'Vamos continuar sua escolha. Suas preferências anteriores foram retomadas.';
            return { sessionId: existingSessionId, restored: true, greeting };
          }
        } catch (e) {
  ...
  async sendMessage(content) {
    ...
    // Conversation text can contain personal details and must not enter browser logs.
  ...
  async sendAction(action) {
    ...
    if (action === 'OPEN_WHATSAPP') {
      const whatsappUrl = this.safeWhatsappUrl(this.lastHandoff?.waLink);
  ...
  formatMessage(content) {
    ...
    // Escape before adding our own formatting so model/user text cannot execute HTML.
    return (
  ...
  safeWhatsappUrl(candidate) {
    if (typeof candidate !== 'string') return null;
    try {
      const url = new URL(candidate);
      if (url.protocol !== 'https:' || url.username || url.password || url.port) return null;
      if (url.hostname === 'wa.me' && /^\/\d{8,15}$/.test(url.pathname)) return url.href;
  ...
  /** @param {unknown} action @returns {boolean} */
